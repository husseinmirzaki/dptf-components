<template>
  <div
    ref="floatingFilter"
    class="position-relative float-start floating-filter"
    style="height: 20px; width: 20px; margin-left: -20px"
  >
    <div
      class="filter-place-holder d-flex align-items-center justify-content-start ps-1"
      :class="{ show: show, active: show || !valueIsEmpty }"
      ref="root"
    >
      <i class="fas fa-filter" @click="toggle()"></i>
      <div
        class="fixed-filter-container"
        ref="filterContainer"
        @move.prevent.stop
        @drag.prevent.stop
        @mousedown.stop
