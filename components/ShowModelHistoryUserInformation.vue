                ...
                ...data.value.map((commitInfo) => {
                  return DEFAULT_COLS.col3(
                      h(Card, {
                        ...
                        onClick: () => {
                          VueInstanceService.emit('show-history-info', commitInfo.id);
                        }
                      }, {
                        'card-content': () => [
                          h(
                              'p', {class: 'my-1'},"کاربر : " + fullNameGenerator(commitInfo.user)
                          ),
                          h(
                              'p', {class: 'my-1'},"عملیات : " + (commitInfo.action == 'added' ? 'اضافه کردن' : commitInfo.action == 'deleted'? 'حذف کردن' : 'ویرایش')
                          ),
                        ],
                        'card-footer': () => [
                            h(
                                'p', {class:'text-center p-1 m-0'},gregorianIOSToPersianDate(commitInfo.date_created, true)
                            )
                        ]
                      }),
                  );
