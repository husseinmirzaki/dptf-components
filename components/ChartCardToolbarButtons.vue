<template>
  <div class="d-flex" :class="{'flex-column': horizontal}">
    ...
    >روزانه</a
    >
    <div class="vertical-divider" v-if="!horizontal && localOptions.showMapToggle"></div>
    <div class="separator mb-3 opacity-75" v-if="horizontal && localOptions.showMapToggle"></div>
    <a v-if="localOptions.showMapToggle"
       :class="{
          active: localOptions.toggleState == 'map'
        }"
       @click="localOptions['toggleState'] = 'map'"
       class="
            btn btn-sm btn-color-muted btn-active btn-active-warning
            px-4
            me-1
          "
    >نقشه</a
