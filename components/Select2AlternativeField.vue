<template>
  <div class="select2-alternative m-0 p-0">
    <div class="row text-center w-100 bg-light m-0 position-relative" :class="{'p-3': !searching, 'p-1': searching}">
      <template v-if="!searching">
        <div class="col" v-for="header in config.headerTranslates" :key="header" @click="searching=true">
          <span>{{ header }}</span>
        </div>
    ...
    <div class="table-body" style="height: 350px;overflow-y: scroll" @scroll="onBodyScroll">
      <div
        ...
        <div class="col d-flex align-items-center justify-content-center" v-for="header in config.headers"
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const onBodyScroll = (e) => {
      if (e.target.offsetHeight + e.target.scrollTop + 60 >= e.target.scrollHeight && !loading.value && !finished.value) {
        page++;
        getData();
      }
    }
    ...
    const processText = (item: any, header: string | number | ((x: any) => any)) => {
