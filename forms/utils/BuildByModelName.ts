...
class BuildByModelName {
    ...
    buildFields(data: any, buildExtend = true) {
        ...
        if (this.onOrderField) {
            ...
            fields = this.onOrderField(
                fields.map((field) => ({name: field.name, field})),
                this
            ).map((fieldName) => {
                ...
                if (_this.onBeforeSubmit) {
                    return (async () => {
                        ...
                        if (_this.onAfterSubmit) {
                            submit = _this.onAfterSubmit(
                                [submit, event, isCustom, onDone],
                                this
                            );
                        }
                        return submit;
        ...
        if (buildExtend) this.formInstance = new CreateFormExtend().extend();
