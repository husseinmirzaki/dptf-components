...
export class ForgottenPasswordForm extends CreateForm {
    ...
    forgotten() {
        return new Promise<void>((resolve, reject) => {
            const dispatch = VueInstanceService.store.dispatch(Actions.FORGOT_PASSWORD, this.refs['basic'].value);
            dispatch.then((data) => {
                VueInstanceService.store.commit(Mutations.SET_FORGOTTEN_USER, this.refs['basic'].value['username'])
