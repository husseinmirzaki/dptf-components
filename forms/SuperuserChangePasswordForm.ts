...
export default class SuperuserChangePasswordForm extends ResetPasswordForm {
    getService(): any {
        return UserApiService;
    }

    change_password(event, userId) {
        return new Promise((resolve, reject) => {
            const data = super.submit(event, true);
            this.getService().superuserChangePassword(userId, data).then((e) => {resolve(e)}, (e) => reject(e));
        });
    }
    ...
    concatFields(): Array<FieldComponentPropsInterface> {
        return [
            ...
            },
            {
                ...
                col_class: 'col-lg-12 col-xl-6 mt-2',
                validation: [
                    Yup.string().required().oneOf([Yup.ref("new_password")], "رمزعبور و تکرار آن باید یکی باشد").label('تکرار رمز عبور'),
                ]
            },
        ];
    }
