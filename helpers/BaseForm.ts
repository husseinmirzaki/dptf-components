...
export class CreateForm<T extends FieldsInterface = any> {
    ...
    set theActiveMode(mode) {
        ...
        if (this.activeMode.value != mode) this.activeMode.value = mode;
    }
    ...
    static optionToSelect2Option(
        ...
        Object.keys(option).forEach((key) => {
            extracted.push([
                key,
                option[key],
                selectedValue && option[key] == selectedValue,
            ]);
        });
        return extracted;
    }
    ...
    processFields() {
        this.concatFields().forEach((e) => {
            ...
            this.modes.forEach((mode) => {
                if (
                    e.onlyOnModes &&
                    e.onlyOnModes.findIndex(this.filterMode(mode)) == -1
                )
                    return;
                if (
                    e.excludeOnModes &&
                    e.excludeOnModes.findIndex(this.filterMode(mode)) > -1
                )
                    return;
            ...
            if (field && (r[e1] instanceof FileList || field.isFile)) {
                for (let i = 0; i < r[e1].length; i++) {
                    if (field) {
                        ...
                        else files[field.name] = r[e1][i] as any;
                    }
                }
            }
        });
            ...
            if (d) {
                ...
                const addValidationToModes = (field, validation) => {
                    this.modes.forEach((mode) => {
                        ...
                        if (
                            excludeModes &&
                            excludeModes.findIndex(this.filterMode(mode)) > -1
                        )
                            return;
    ...
    extend(): CreateFormExtend<CreateForm & this, T> {
        ...
        return {
            ...
            activeSchema: computed(
                () => this.validationSchemas[this.activeMode.value]
            ),
                ...
                Object.keys(this.refs[refKey].value).forEach((dataKey) => {
                    ...
                    if (field && field.field_type === "select") {
                        try {
                            ...
                            else this.elementRefs[dataKey].select2Instance.value.val(null);
    ...
    public static buildSeparator(title, options: any | undefined = undefined): FieldComponentPropsInterface {
        return {
            name: randomId(),
    ...
    buildSeparator(title, options: any | undefined = undefined): FieldComponentPropsInterface {
        return CreateForm.buildSeparator(title, options);
    }
}
